...
const generateExperienceHTML = (experiences) => {
  ...
  experiences.forEach((exp, index) => {
    console.log(`Experience ${index + 1}: ${exp.company} - ${exp.job_title}`);
    console.log('Description:', exp.description);
    console.log('Description length:', exp.description?.length || 0);
    console.log('---');
  });
...
const generateEducationHTML = (education) => {
  console.log('🎓 ===== EDUCATION DESCRIPTIONS =====');
  education.forEach((edu, index) => {
    console.log(`Education ${index + 1}: ${edu.institution}`);
    console.log('Description:', edu.description);
    console.log('Description length:', edu.description?.length || 0);
    console.log('---');
  });
...
const mainContent = computed(() => {
  ...
  if (props.templateConfig.paginatorType !== 'oneColHeader' && props.templateConfig.paginatorType !== 'oneColHeaderOnce') {
    return ''
  }
